...
angular.module('BMCModule').controller('PlayerListCtrl',
	['apiService', 'loadINI', '$location', '$rootScope', '$scope', '$filter', '$modal', '$timeout', '$log', "$compile", "$window", 'localStorageService', 'requestNotificationChannel', 'PlayerService', '$q', 'utilsSvc',
		function (apiService, loadINI, $location, $rootScope, $scope, $filter, $modal, $timeout, $log, $compile, $window, localStorageService, requestNotificationChannel, PlayerService, $q, utilsSvc) {
			...
			var config = null;
			// try to get studio config from BMC (should work if we are in BMC)
			try {
				var bmc = window.parent.bmc;
				if (bmc && bmc.vars && bmc.vars.studio.config) {
					config = bmc.vars.studio.config;
					$scope.UIConf = angular.fromJson(config);
				}
			} catch (e) {
				// standalone version
				$log.error('Could not located parent.bmc: ' + e);
			}

			// if we didn't get the uiconf from bmc.vars - load the configuration from base.ini
			if (!config) {
			...
			// get players list from BMC
			var playersRequest = {
				'filter:tagsMultiLikeOr': 'bdp3,html5studio',
				'filter:orderBy': '-updatedAt',
				'filter:objTypeIn': '1,8',
				'filter:objectType': 'BorhanUiConfFilter',
				'filter:creationModeEqual': '2',
				'ignoreNull': '1',
				'responseProfile:objectType': 'BorhanDetachedResponseProfile',
				'responseProfile:type': '1',
				'responseProfile:fields': 'id,name,html5Url,createdAt,updatedAt,tags',
				'page:objectType': 'BorhanFilterPager',
				'pager:pageIndex': '1',
				'pager:pageSize': '999',
				'service': 'uiConf',
				'action': 'list'
			...
			$scope.upgrade = function (player) {
				...
				msg += '<br>Update to version: ' + currentVersion + '<a href="https://github.com/borhan/mwEmbed/releases/tag/v' + currentVersion + '" target="_blank"> (release notes)</a>';
					...
					if (result) {
						...
						PlayerService.playerUpgrade(player, html5lib).then(function (data) {
							...
							if ($("#kcms", window.parent.document).length > 0) {
								$("#kcms", window.parent.document)[0].refreshPlayersList(); // trigger players list refresh on BMC
							}
						}, function (reason) {
